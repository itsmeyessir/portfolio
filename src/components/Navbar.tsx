...
export function Navbar() {
  ...
  const { fastDuration } = useRefreshRateContext();
  ...
  return (
    <nav
      className="fixed top-0 left-0 w-full z-[100] bg-[#101014]/20 backdrop-blur-md transition-all"
      style={{ height: "auto" }}
    >
      <div className="w-full max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link
            href="/"
            className="text-sm sm:text-base md:text-lg font-bold tracking-tight text-white hover:text-neutral-300 transition-colors"
          >
            itsmeyessir
          </Link>
        </div>
        ...
        >
          <div className="w-6 h-6 flex flex-col justify-center items-center gap-[5px]">
            <span
              className={`w-4 h-[2px] bg-white rounded-full transition-all duration-300 ease-[cubic-bezier(.47,1.64,.41,.8)] transform origin-center ${sidebarOpen ? "rotate-45 translate-y-[3.5px]" : ""}`}
            ></span>
            <span
              className={`w-4 h-[2px] bg-white rounded-full transition-all duration-300 ease-out ${sidebarOpen ? "opacity-0 scale-x-0" : "opacity-100"}`}
            ></span>
            <span
              className={`w-4 h-[2px] bg-white rounded-full transition-all duration-300 ease-[cubic-bezier(.47,1.64,.41,.8)] transform origin-center ${sidebarOpen ? "-rotate-45 -translate-y-[3.5px]" : ""}`}
            ></span>
        ...
        <ul className="hidden md:flex gap-4 md:gap-6 lg:gap-8 relative">
          {navLinks.map((link) => {
            ...
            const isActive = linkId ? active === linkId : false;
            return (
        ...
        {sidebarOpen && (
          <div className="fixed inset-0 z-[999]">
            ...
            <motion.div
              ...
              style={{
                background:
                  "linear-gradient(145deg, rgba(12, 12, 14, 0.97) 0%, rgba(18, 18, 22, 0.97) 100%)",
            ...
            >
              <div className="flex flex-col min-h-screen h-full p-6">
                ...
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-white font-medium text-lg flex items-center h-10">
                    Menu
                  </h2>
                  ...
                  {navLinks.map((link) => {
                    ...
                    return (
                      <a
                        ...
                        onClick={(e) => {
                          ...
                          if (linkId) {
                            ...
                            // Standard link navigation for paths like /blog
                            setSidebarOpen(false);
                          }
                        }}
                      ...
                      >
                        ...
                        <span className="text-base font-medium block text-left pl-2">
                          {link.name}
                        </span>
                      </a>
                    );
